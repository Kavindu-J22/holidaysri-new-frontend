...
const AddForm = (props) => {
  ...
  const [promoCode, setPromoCode] = useState(""); // New state for promoCode
  ...
  // const userPromoCode = location.state?.promoCode;
  ...
  // Get promoCode from previous page state if passed
    useEffect(() => {
      if (status.promoCode) {
        setPromoCode(status.promoCode);
      }
    }, [status.promoCode]);
  ...
  const handleAddVehicle = async () => {
    try {
      ...
      if (editingHotel) {
        ...
        await axios.post(
          "https://holidaysri-backend.onrender.com/hotel/add",
          vehicleData
        );
      ...
      alert(
        "Error occurred. An image upload is required. Please upload an image ",error
      );
    }
  };
    ...
    >
      ...
      >
       ...
       <div>
         ...
         <Typography variant="h6">Promo Code: {promoCode}</Typography>
