...
import { Grid, Box, Button, Typography, Modal, Card,
  CardContent,
...
const filteredVehicles = () => {
  const params = new URLSearchParams(location?.search);
  const category = params?.get('category');
  const driverStatus = params?.get('driverStatus');
  const currentDate = new Date(); // Get the current date

  console.log('Category:', category);
  console.log('Driver Status:', driverStatus);

  const isNotExpired = (vehicle) => {
    if (!vehicle.expirationDate) {
      // If there's no expiration date, consider it valid
      return true;
    }
    
    const expirationDate = new Date(vehicle.expirationDate);
    // Return true if the expiration date is greater than the current date
    return expirationDate > currentDate;
  };

  if (category && driverStatus) {
    // Filter based on both category, driver status, and expiration date
    return vehicleDetails.filter(vehicle => {
      console.log('Vehicle:', vehicle);
      return (
        vehicle.Vehiclecategory.toLowerCase() === category.toLowerCase() &&
        vehicle.driverStatus.toLowerCase() === driverStatus.toLowerCase() &&
        isNotExpired(vehicle) // Only show not expired vehicles
      );
    });
  } else if (category) {
    // Filter based only on category and expiration date
    return vehicleDetails.filter(vehicle => {
      console.log('Vehicle:', vehicle);
      return (
        vehicle.Vehiclecategory.toLowerCase() === category.toLowerCase() &&
    ...
    // Filter based only on driver status and expiration date
    return vehicleDetails.filter(vehicle => {
      console.log('Vehicle:', vehicle);
      return (
        vehicle.driverStatus.toLowerCase() === driverStatus.toLowerCase() &&
    ...
    // Return all vehicles that are not expired if no filters are applied
    return vehicleDetails.filter(vehicle => {
      console.log('Vehicle:', vehicle);
      return isNotExpired(vehicle); // Only show not expired vehicles
    });
  }
};
              ...
              {selectedEvent && (
                <>
                  ...
                  <Typography
                    ...
                    color="white"
                  >
                    AC : {selectedEvent.ac?"Yes":"No"}
                  </Typography>
                  </Grid>
                  <Grid item lg={4} xs={12} marginTop={{lg:'0px',xs:'8px'}}>
                    {selectedEvent.capacity && <>  <Typography
                    id="modal-modal-title"
                    variant="body"
                    color="white"
                  >
...
export default AllvehiclesCategory;
